...
- A global Web Audio BGM provider maps 15 playable route families to 13 original procedural themes. Music is disabled by default, starts only after a user gesture, crossfades when the active game theme changes, fades while the tab is hidden, and stores one global volume preference.
