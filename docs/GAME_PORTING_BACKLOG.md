...
- A global Web Audio BGM provider maps 15 playable route families to 13 original procedural themes. Every theme uses a 256-step, 16-bar arrangement with at least five intro/A/break/B/return/finale sections instead of a short repeating phrase. Peak sections now expand to 12 scheduled roles: lead, harmony lead, octave lead, counter melody, arpeggio, bass, inverted/extended pads, kick, snare, hi-hat, percussion, and transition FX. Section impacts, noise-and-pitch risers, filter sweeps, rhythmic chord stabs, open-hat/tom fills, stereo placement, delay, synthesized reverb, and kick-driven musical pumping make the B themes and finales materially larger than their intros. Music is disabled by default, starts only after a user gesture, crossfades when the active game theme changes, fades while the tab is hidden, stores one global volume preference, and ducks under gameplay SFX so action feedback remains legible.
