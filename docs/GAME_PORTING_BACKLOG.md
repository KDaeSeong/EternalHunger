...
- A global Web Audio BGM provider maps 15 playable route families to 13 original procedural themes. Every theme uses a 288-step, 18-bar arrangement with at least six intro/A/break/B/return/climax/finale sections instead of a short repeating phrase. Peak sections now expand to 18 scheduled roles: lead, harmony lead, octave lead, counter melody, arpeggio, bass, inverted/extended pads, string ostinato, brass stabs, bell accents, choir pads, sub-bass, kick, snare, hi-hat, percussion, tom fills, and transition FX. Game-style orchestration palettes, a dedicated orchestral mix bus, section impacts, noise-and-pitch risers, filter sweeps, rhythmic chord stabs, stereo placement, delay, synthesized reverb, and kick-driven musical pumping make each climax materially larger than its intro. Music is disabled by default, starts only after a user gesture, crossfades when the active game theme changes, fades while the tab is hidden, stores one global volume preference, and ducks under gameplay SFX so action feedback remains legible.
...
- Tutorial-supported games share one fixed utility dock with icon-only SFX/BGM toggles and a compact music-volume popover, keeping audio controls reachable without expanding each game header.
