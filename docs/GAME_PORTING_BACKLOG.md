...
- A global Web Audio BGM provider maps 15 playable route families to 13 original procedural themes. Every theme now uses a 256-step, 16-bar arrangement with at least five intro/A/break/B/return/finale sections instead of a short repeating phrase. Eight scheduled layers cover lead, counter melody, arpeggio, bass, pad, kick, snare, hi-hat, and percussion roles, with per-theme swing, filters, stereo placement, delay, and synthesized reverb. Music is disabled by default, starts only after a user gesture, crossfades when the active game theme changes, fades while the tab is hidden, stores one global volume preference, and ducks under gameplay SFX so action feedback remains legible.
