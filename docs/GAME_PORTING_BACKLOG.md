...
- Research map routing: nodes are ordered by survival/camp/craft/science/culture/spiritual/military/civics lanes and packed within each tier. Adjacent dependencies use the gutter between columns, skipped-tier dependencies use a faint dashed header bus, and focused dependencies receive the only high-emphasis route. Browser geometry checks confirm that none of the 111 dependency paths crosses an unrelated node.
