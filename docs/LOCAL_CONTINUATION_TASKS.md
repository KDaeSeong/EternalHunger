...
- Shared game audio now includes 13 original procedural BGM profiles covering 15 route families. `GameBgmProvider` keeps music off by default, unlocks Web Audio on an explicit gesture, crossfades route changes, fades hidden tabs, and persists global enable/volume preferences. Tutorial-supported routes expose SFX, BGM, and music volume through one compact fixed utility dock; `check:game-feedback-shell` validates the route/profile/preference contracts.
