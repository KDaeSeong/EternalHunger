...
- Primitive Archive development now separates 126 technologies from 76 social institutions. The Growth tab exposes a `Technology / Civics` switch: technologies use RP and Eureka, civics use CP and Inspiration, while cross-tree prerequisites remain visible and enforceable. Legacy unified research saves migrate civic progress into the civics state during normalization.
- Primitive Archive research spans T1-T30 and C1-C26. The classical era has 17 technologies and 13 social institutions, while the medieval, early-modern, and early modern-period expansions each add 24 technologies and 18 social institutions across faith, natural philosophy, literature, military, survival, and engineering branches. The early modern-period layer adds industrial power, modern science and medicine, mass print, national military organization, public education, sanitation, labor protection, liberalism, and humanitarian institutions. Every technology has an eureka and every social institution has an inspiration. Same-track prerequisites always point to a lower stage, cross-track requirements remain explicit inspector gates, automatic target selection favors the lowest available stage, and the record branch progresses through mass printing and telegraphy into mass publishing. Campfire + shelter + workbench Lv.1 unlock target selection and turn/day automatic RP; completing T1 Gathering + Hunting unlocks the manual research action.
- Primitive Archive's research map shows track-specific T/C headers with stage names and node counts, includes those labels in nodes/dropdowns/recommendations, and keeps only the map canvas horizontally scrollable on narrow screens. Before research unlocks, `PrimitiveArchiveResearchTreePreview.js` exposes all 202 advancements, prerequisites, follow-ups, eureka/inspiration conditions, and unlock effects as read-only T1-T30 and C1-C26 trees; the gate includes a direct jump button while target assignment and research remain locked. Classical, medieval, early-modern, and early modern-period nodes expose branch-specific text and icons, and the first completion in each later era triggers a dedicated era-advance sound cue.
- Primitive Archive advancement lore applies 60 compact Civilization-style display-name overrides without changing save IDs. All 202 technology and civic nodes receive an explicit topic-matched quotation with author, work, and source URL. The 38-theme, 33-source map has no generic fallback or duplicate assignment and is rendered in both the locked tree preview and live research inspector. Dedicated guards cover tools, pottery, poetry, steam power, utilitarianism, food preservation, refrigeration, optics, empiricism, art, music, tolerance, and electricity.
- Primitive Archive research layout is branch-aware and compact per stage. Early technology density is 2/4/4/3 across T1-T4, dependency paths use orthogonal column gutters, skipped-stage links use a low-emphasis dashed header bus, and selected-node links override those styles for inspection. The 373 full prerequisites split into 146 rendered technology links, 94 rendered civic links, and 133 cross-track inspector gates; automated geometry checks prove that rendered paths do not cross unrelated nodes. Long conceptual progressions such as Hunting -> Military Tradition -> Trapping are shown as cross-track follow-ups instead of forced lines across unrelated technology columns.
...
- BA Vanguard now uses a compact hero, nine primary micro metrics, a minimal duel coach, a pinned latest result, icon-led feature tabs, and icon controls for save/load/record/replay/room/detail commands. `baVanguardFeedback.js` maps duel snapshots to 23 result states backed by 22 distinct cues, including call/retire, deck out, and replay feedback; the duel and hand tabs repeat the shared latest result next to their controls. `check-ba-vanguard-feedback.mjs` drives real engine transitions for phase, draw, ride, call, retire, stride, skill, attack, guard, perfect guard, block, hit, damage, turn, victory, defeat, and deck out, and verifies the loaded-state baseline wrapper, automatic-AI silence, and click-sound suppression. Production-browser verification covered new duel, invalid AI progress, draw, phase, successful ride, result icons/tones, zero console errors, and a 375px content viewport with no page-level horizontal overflow or clipped control labels.
...
2. In progress: `client/src/app/games/schale-idle-rpg/play/page.js`
   ...
   - Next: split `SchaleIdleGearTab.js` into vault/tuning/preset/salvage panels before adding the original enhancement protection and pity policy.
