...
- Primitive Archive research now has 60 connected Primitive/Neolithic/Ancient/Classical nodes arranged into explicit T1-T12 stages across the two tracks. Every prerequisite points to a lower tier, automatic target selection favors the lowest available tier, and the record branch now progresses Writing -> Clay Tablets/Bookcraft -> Library. Campfire + shelter + workbench Lv.1 unlock target selection and turn/day automatic RP; completing T1 Gathering + Hunting unlocks the manual research action.
- Primitive Archive's research map shows sticky tier headers with stage names and node counts, includes tier labels in nodes/dropdowns/recommendations, and keeps only the map canvas horizontally scrollable on narrow screens. Before research unlocks, `PrimitiveArchiveResearchTreePreview.js` exposes all 60 T1-T12 nodes, prerequisites, follow-ups, eureka/inspiration conditions, and unlock effects as a read-only tree; the gate includes a direct jump button while target assignment and research remain locked.
- Primitive Archive research layout is branch-aware and compact per tier. Dependency paths use orthogonal column gutters, skipped-tier links use a low-emphasis dashed header bus, and selected-node links override those styles for inspection. Both the locked preview and active growth tree expose node/edge IDs for browser geometry checks; the current 60-node/111-edge graph has zero unrelated-node path intersections.
