...
- School Simulator now uses a compact hero/advisor/result shell, dense responsive stat grids, and nested advanced tabs for weekly operations, people/events, vision/incidents, and reports. Header and care-board commands use shared icon controls. Its dedicated feedback classifier gives policy changes, week bells, incidents, resolutions, crises, recoveries, festivals, and semester rollover separate Lucide result states and synthesized cues, covered by `check:school-simulator-feedback`.
...
- Primitive Archive development now separates 46 technologies from 14 civics. The Growth tab exposes a `Technology / Civics` switch: technologies use RP and Eureka, civics use CP and Inspiration, while cross-tree prerequisites remain visible and enforceable. Legacy unified research saves migrate civic progress into the new civics state during normalization.
- Primitive Archive research now separates 102 technologies into T1-T22 and 58 social institutions into C1-C20. The classical era has been rebalanced to 17 technologies and 13 social institutions, while both the medieval and early-modern expansions add 24 technologies and 18 social institutions across faith, natural philosophy, literature, military, survival, and engineering branches. Every technology has an eureka and every social institution has an inspiration. Same-track prerequisites always point to a lower stage, cross-track requirements remain explicit inspector gates, automatic target selection favors the lowest available stage, and the record branch progresses Writing -> Clay Tablets/Bookcraft -> Library -> Cathedral Schools/Papermaking -> Printing Blocks/University Tradition -> Movable Type/Print Workshop. Campfire + shelter + workbench Lv.1 unlock target selection and turn/day automatic RP; completing T1 Gathering + Hunting unlocks the manual research action.
- Primitive Archive's research map shows track-specific T/C headers with stage names and node counts, includes those labels in nodes/dropdowns/recommendations, and keeps only the map canvas horizontally scrollable on narrow screens. Before research unlocks, `PrimitiveArchiveResearchTreePreview.js` exposes all 160 advancements, prerequisites, follow-ups, eureka/inspiration conditions, and unlock effects as read-only T1-T22 and C1-C20 trees; the gate includes a direct jump button while target assignment and research remain locked. Classical, medieval, and early-modern nodes expose branch-specific text and icons, and the first medieval or early-modern completion triggers a dedicated era-advance sound cue.
- Primitive Archive advancement lore now applies 60 compact Civilization-style display-name overrides without changing save IDs. All 160 technology and civic nodes receive a topic-matched public-domain quotation with author, work, and source URL, rendered in both the locked tree preview and live research inspector.
- Primitive Archive research layout is branch-aware and compact per stage. Early technology density is 2/4/4/3 across T1-T4, dependency paths use orthogonal column gutters, skipped-stage links use a low-emphasis dashed header bus, and selected-node links override those styles for inspection. The 289 full prerequisites split into 121 rendered technology links, 76 rendered civic links, and 92 cross-track inspector gates; automated geometry checks prove that rendered paths do not cross unrelated technology columns. Long conceptual progressions such as Hunting -> Military Tradition -> Trapping are shown as cross-track follow-ups instead of forced lines across unrelated technology columns.
