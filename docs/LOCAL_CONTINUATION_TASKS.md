...
- School Simulator now uses a compact hero/advisor/result shell, dense responsive stat grids, and nested advanced tabs for weekly operations, people/events, vision/incidents, and reports. Header and care-board commands use shared icon controls. New policy, counseling, lesson, festival, and semester semantics map to dedicated Lucide objects and synthesized cues; week rollover emits separate event, festival-completion, and semester stingers.
