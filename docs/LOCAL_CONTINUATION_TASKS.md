...
- Shared game audio now includes 13 original procedural BGM profiles covering 15 route families. Each profile has a 288-step, 18-bar arrangement with at least six named sections, and its climax can use 18 roles spanning A/B lead material, harmony and octave doubles, counter melody, arpeggio, bass, inverted/extended pads, string ostinato, brass stabs, bell accents, choir pads, sub-bass, five drum roles, and transition FX. `GameBgmProvider` gives the added orchestration a dedicated mix bus and game-style palette, then combines it with section impacts, noise-and-pitch risers, filter sweeps, open-hat/tom fills, kick-driven pumping, stereo placement, delay, synthesized reverb, route crossfades, hidden-tab fades, and SFX-triggered ducking while preserving gesture unlock and global enable/volume preferences. Tutorial-supported routes expose SFX, BGM, and music volume through one compact fixed utility dock; `check:game-feedback-shell` validates arrangement length, section integrity, all 18 roles, climax orchestration, extended voicings, theme variety, and ducking contracts.
