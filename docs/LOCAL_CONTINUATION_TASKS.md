...
- Shared game audio now includes 13 original procedural BGM profiles covering 15 route families. Each profile has a 256-step, 16-bar arrangement with at least five named sections, separate A/B lead material, counter melody, arpeggio, bass, pads, and four drum roles. `GameBgmProvider` adds section-aware filters, stereo placement, delay, synthesized reverb, route crossfades, hidden-tab fades, and SFX-triggered ducking while preserving gesture unlock and global enable/volume preferences. Tutorial-supported routes expose SFX, BGM, and music volume through one compact fixed utility dock; `check:game-feedback-shell` now validates arrangement length, section integrity, layer presence, harmonic roots, theme variety, and ducking contracts.
