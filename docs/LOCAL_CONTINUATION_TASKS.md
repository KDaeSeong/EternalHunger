...
- Shared game audio now includes 13 original procedural BGM profiles covering 15 route families. Each profile has a 256-step, 16-bar arrangement with at least five named sections, and its climax can use 12 roles spanning A/B lead material, harmony and octave doubles, counter melody, arpeggio, bass, inverted/extended pads, four drum roles, and transition FX. `GameBgmProvider` adds section impacts, noise-and-pitch risers, filter sweeps, rhythmic chord stabs, open-hat/tom fills, kick-driven pumping on a separate music bus, stereo placement, delay, synthesized reverb, route crossfades, hidden-tab fades, and SFX-triggered ducking while preserving gesture unlock and global enable/volume preferences. Tutorial-supported routes expose SFX, BGM, and music volume through one compact fixed utility dock; `check:game-feedback-shell` validates arrangement length, section integrity, all 12 roles, climax flourishes, extended voicings, theme variety, and ducking contracts.
