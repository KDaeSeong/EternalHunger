...
- Eternal Hunger now has nine original procedural soundtrack scenes: ready, day, night, combat, dimensional rift, mutant boss, final zone, result, and defeat. The loops span 32-48 bars (about 71-105 seconds), rotate A/B/C motifs by phrase, use eight-bar chord variations and late key shifts, and crossfade from phase ambience into event-specific rift, boss, sudden-death, victory, or defeat cues. Its dedicated palettes use the shared 27-role score, including string ensemble, synth pulse, supersaw lead stacks, synth plucks, guitar stabs, timpani, hybrid impacts, ghost snare, and ride cymbal. Battle tracks run at 138-154 BPM while result and defeat tracks retain wider dynamic drops and longer crossfades. No reference samples or melodies are stored in the repository. `npm run check:eternal-hunger-soundtrack` validates all nine profiles and state wiring.
...
- Eternal Hunger inventory normalization now merges duplicate starting material stacks without dropping goal tags, while high-tier and named special materials remain protected during automatic space recovery. `npm run check:eternal-hunger` is the completion gate for inventory/economy regressions, revival, custom rosters, minimap geometry, weapons, skills, event feedback, soundtrack, runtime lint, and the production build.
...
- Dual Academy TCG now uses a compact turn coach, an icon-led live duel event pulse, explicit header/deck/board action icons, two-column mobile match metrics, and accessible card-count controls. `dualAcademyTcgFeedback.js` maps match snapshots to the shared duel cues plus dedicated Mika cost/negate/burst, Hina discipline/recovery, and Yuuka guard/search feedback. The route has a dedicated `academy-duel` stereo/reverb effects palette and ten original 28-44 bar dynamic OST scenes for ready, main, battle, chain, danger, Mika, Hina, Yuuka, victory, and defeat. All loops exceed 65 seconds and use three rotating motifs, four-note harmony, eight-bar progression, late key shifts, strings, pulse synth, cymbals, and ghost snare. Tabs and duel pressure choose the base track while confirmed feedback cues temporarily crossfade to action or character music; `check:dual-academy-soundtrack` protects profile depth and runtime wiring. The engine ports Mika's costed quick negate and battle boost, Hina's battle recovery, and Yuuka's turn-based target protection with deterministic regression coverage. Client-only duel rendering also prevents random match/event IDs from causing character-quote hydration mismatches.
...
- Primitive Archive now exposes actions, map, party, camp/report, archive, research, tribe, inventory, and run management in one top-level tab row. The latest result is inside the action dock, the turn forecast follows the controls, and difficulty/record/settlement controls moved into run management. The 1920x1080 desktop density pass removes the action dock's erroneous 82px sticky offset, caps the map canvas at 800x520, prevents grid panels from stretching their contents to sibling height, uses three-column archive reports and rival-tribe cards, and compacts party, camp, report, tribe, and inventory cards. Every photographed tab now fits the 842px internal workspace without document scrolling; the 150-technology/94-civic Growth tree intentionally keeps bounded internal scrolling. At 390x844, hero controls, metrics, feature tabs, and the map use contained horizontal scrolling with no page-level horizontal overflow.
...
- The shared audio popover now displays the active soundtrack title and its object-only scene icon. Every base BGM profile and all dedicated dynamic soundtracks expose icon metadata, while the compact fixed utility dock keeps this added context out of the game viewport until opened.
...
- Tonkatsu Teacher now has eight original dynamic soundtrack scenes for opening, kitchen work, service, growth, contests, judging, celebration, and setbacks. Controlled feature tabs select persistent music while the existing 24 management result classes trigger timed scene crossfades. All 28-44 bar loops exceed 65 seconds, rotate A/B/C motifs, use four-note harmony and late key shifts, and activate the expanded synth-pluck, guitar, timpani, and hybrid-impact layers. Eight object-only scene icons identify the current score; `npm run check:tonkatsu-teacher-soundtrack` verifies profiles, priority, transitions, controlled tabs, and icons.
...
npm run check:dual-academy-soundtrack
npm run check:tonkatsu-teacher-soundtrack
