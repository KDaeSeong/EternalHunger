...
세부 계약·전체 수치·시간 초과된 팀 선택의 한계는 `observer-frame-yield-verification.md`, 원자료는 `artifacts/observer-frame-yield-browser-20260924.json`이다. **최신 x1·동일 조건 3회 시작 heap/자료 수명·회수는 남는다.** 새 재경기 두 번의 시작 heap도 31.83→35.62MB로 증가했고, 처음 로딩한 원본과 서로 다른 배속의 재경기를 기존 메모리 관문의 대체 증거로 쓰지 않는다. G4/HF1·전체 목표는 미완료이며 다음 절의 이전 실패 근거를 보존한다.
