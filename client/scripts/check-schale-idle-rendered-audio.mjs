...
for (const instrument of [
  'vibraphone', 'bell', 'violin', 'viola', 'cello', 'flute', 'oboe', 'bassoon', 'horn',
  'trumpet', 'trombone', 'harp', 'celesta', 'strings', 'timpani', 'orchestral-snare', 'cymbal', 'tom',
]) {
...
assert.match(
  rendererSource,
  /theme: 'idle-workshop'[\s\S]*lead: 'celesta'[\s\S]*counter: 'bell'[\s\S]*woodwind: 'bassoon'/,
  '공방 장면에는 첼레스타·벨·바순 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'idle-tower'[\s\S]*lead: 'trumpet'[\s\S]*pad: 'strings'/,
  '시련의 탑 장면에는 트럼펫과 현악 편성이 필요합니다.',
);
assert.match(
  rendererSource,
