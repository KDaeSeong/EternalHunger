...
import {
  RAIL3D_SOUNDTRACK,
} from '../src/app/games/rail3d-sim/_lib/rail3dSoundtrack.js';

const clientRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const publicRoot = path.join(clientRoot, 'public');
const rendererSource = await readFile(
  path.join(clientRoot, 'scripts', 'render-rail3d-instrumental.mjs'),
  'utf8',
);
...
const manifest = JSON.parse(await readFile(
  path.join(publicRoot, 'audio', 'rail3d-sim', 'manifest.json'),
  'utf8',
));
...
assert.equal(manifest.tracks.length, 6, 'Rail3D 렌더 트랙은 6곡이어야 합니다.');
assert.equal(
  Object.keys(GAME_BGM_RENDERED_TRACKS).filter((theme) => theme.startsWith('rail-')).length,
  6,
  'Rail3D 렌더 트랙 매핑은 6곡이어야 합니다.',
);
assert.equal(new Set(manifest.tracks.map((track) => track.file)).size, 6);
...
let sampledEvents = 0;
for (const soundtrackRow of RAIL3D_SOUNDTRACK) {
  ...
  assert.match(renderedTrack.src, /^\/audio\/rail3d-sim\/[a-z-]+\.wav$/);
  ...
  assert.equal(
    durationSeconds >= 60 && durationSeconds <= 95,
    true,
...
assert.equal(
  totalBytes >= 34_000_000 && totalBytes <= 45_000_000,
  true,
  'Rail3D 렌더 음원 용량이 예상 범위를 벗어났습니다.',
);
for (const instrument of [
  'vibraphone', 'bell', 'violin', 'viola', 'cello', 'flute', 'oboe', 'bassoon', 'horn',
  'trumpet', 'trombone', 'harp', 'celesta', 'strings', 'timpani', 'taiko', 'rail-clank',
]) {
...
assert.equal(sampledEvents >= 600, true, '전체 OST에서 실제 연주 샘플 이벤트가 부족합니다.');
assert.match(
  rendererSource,
  /theme: 'rail-yard'[\s\S]*lead: 'celesta'[\s\S]*counter: 'bassoon'/,
  '차량기지에는 첼레스타와 바순 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-mainline'[\s\S]*lead: 'violin'[\s\S]*counter: 'vibraphone'/,
  '본선 운행에는 바이올린과 비브라폰 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-dispatch'[\s\S]*lead: 'bassoon'[\s\S]*counter: 'trumpet'/,
  '관제 장면에는 바순과 트럼펫 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-congestion'[\s\S]*lead: 'trumpet'[\s\S]*counter: 'cello'[\s\S]*pad: 'strings'/,
  '혼잡 장면에는 트럼펫·첼로·현악 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-arrival'[\s\S]*lead: 'horn'[\s\S]*counter: 'bell'[\s\S]*pad: 'strings'/,
  '종착 장면에는 호른·역 벨·현악 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-archive'[\s\S]*lead: 'harp'[\s\S]*counter: 'celesta'[\s\S]*woodwind: 'bassoon'/,
  '운행 기록에는 하프·첼레스타·바순 편성이 필요합니다.',
);
assert.match(rendererSource, /percussion: 'industrial-orchestra'/, '운행 장면에는 산업 오케스트라 타악기가 필요합니다.');
assert.match(sharedRendererSource, /'rail-clank'/, '공용 렌더러에 레일 금속 타악기가 필요합니다.');
