...
for (const instrument of [
  ...
  'trumpet', 'trombone', 'harp', 'celesta', 'choir', 'timpani', 'taiko', 'rail-clank',
]) {
  assert.equal(soundtrackInstruments.has(instrument), true, `전체 편성 악기 누락: ${instrument}`);
}
assert.match(
  rendererSource,
  /theme: 'rail-yard'[\s\S]*lead: 'piano'[\s\S]*counter: 'clarinet'/,
  '차량기지에는 피아노와 클라리넷 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-mainline'[\s\S]*lead: 'violin'[\s\S]*counter: 'guitar'/,
  '본선 운행에는 바이올린과 기타 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-dispatch'[\s\S]*lead: 'clarinet'[\s\S]*counter: 'trumpet'/,
  '관제 장면에는 클라리넷과 트럼펫 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-congestion'[\s\S]*lead: 'trumpet'[\s\S]*counter: 'cello'/,
  '혼잡 장면에는 트럼펫과 첼로 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-arrival'[\s\S]*lead: 'horn'[\s\S]*counter: 'violin'[\s\S]*pad: 'choir'/,
  '종착 장면에는 호른·바이올린·합창 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'rail-archive'[\s\S]*lead: 'piano'[\s\S]*counter: 'celesta'/,
  '운행 기록에는 피아노와 첼레스타 편성이 필요합니다.',
);
