...
import {
  BA_VANGUARD_SOUNDTRACK,
} from '../src/app/games/ba-vanguard/_lib/baVanguardSoundtrack.js';

const clientRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const publicRoot = path.join(clientRoot, 'public');
const rendererSource = await readFile(
  path.join(clientRoot, 'scripts', 'render-ba-vanguard-instrumental.mjs'),
  'utf8',
);
const manifest = JSON.parse(await readFile(
  path.join(publicRoot, 'audio', 'ba-vanguard', 'manifest.json'),
  'utf8',
));
...
assert.equal(manifest.tracks.length, 7, 'BA 뱅가드 렌더 트랙은 7곡이어야 합니다.');
assert.equal(
  Object.keys(GAME_BGM_RENDERED_TRACKS).filter((theme) => theme.startsWith('vanguard-')).length,
  7,
  'BA 뱅가드 런타임 렌더 트랙 매핑은 7곡이어야 합니다.',
);
...
let sampledEvents = 0;
for (const soundtrackRow of BA_VANGUARD_SOUNDTRACK) {
  ...
  assert.match(renderedTrack.src, /^\/audio\/ba-vanguard\/[a-z-]+\.wav$/);
...
assert.equal(totalBytes >= 43_000_000 && totalBytes <= 58_000_000, true, 'BA 뱅가드 렌더 음원 용량이 예상 범위를 벗어났습니다.');
...
assert.match(rendererSource, /lead: 'celesta'/, '트리거 장면에 셀레스타가 필요합니다.');
assert.match(rendererSource, /lead: 'vibraphone'/, '준비 장면에 비브라폰이 필요합니다.');
assert.match(rendererSource, /counter: 'bell'/, '라이드 장면에 벨 편성이 필요합니다.');
assert.match(rendererSource, /woodwind: 'bassoon'/, '패배 장면에 바순 편성이 필요합니다.');
