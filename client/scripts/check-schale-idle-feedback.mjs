...
const dutyImpactState = {
  ...base,
  maxClearedFloor: 2,
  credits: 1080,
  stamina: 88,
  log: ['30분 방치 정산: 2층 클리어, +360 Cr, 보스 0회.'],
};
const dutyImpacts = feedback.schaleIdleFeedbackImpacts(base, dutyImpactState);
assert.deepEqual(dutyImpacts, [
  { action: 'growth', label: '최고 층', value: '+2층', tone: 'success' },
  { action: 'settle', label: '크레딧', value: '+360 Cr', tone: 'success' },
  { action: 'rest', label: '스태미나', value: '-12', tone: 'warning' },
], '방치 정산은 층, 크레딧, 스태미나 변화를 우선순위대로 표시해야 합니다.');

const craftImpactState = {
  ...base,
  credits: 620,
  equipmentInventory: { 'impact-gear': { uid: 'impact-gear' } },
  log: ['녹슨 소총 제작 완료.'],
};
assert.deepEqual(feedback.schaleIdleFeedbackImpacts(base, craftImpactState), [
  { action: 'equip', label: '보유 장비', value: '+1개', tone: 'highlight' },
  { action: 'settle', label: '크레딧', value: '-100 Cr', tone: 'warning' },
], '제작은 새 장비와 지출을 함께 보여줘야 합니다.');

const upgradeId = Object.keys(base.upgrades)[0];
const researchImpactState = {
  ...base,
  credits: 520,
  upgrades: { ...base.upgrades, [upgradeId]: Number(base.upgrades[upgradeId] || 0) + 1 },
  log: ['상시 연구 완료.'],
};
assert.deepEqual(feedback.schaleIdleFeedbackImpacts(base, researchImpactState), [
  { action: 'research', label: '상시 연구', value: '+1 Lv.', tone: 'success' },
  { action: 'settle', label: '크레딧', value: '-200 Cr', tone: 'warning' },
], '상시 연구는 레벨 상승과 비용을 함께 표시해야 합니다.');

const rewardImpactState = {
  ...base,
  credits: 900,
  claimedMissions: ['mission-check'],
  log: ['미션 1개 보상 수령. +180 Cr.'],
};
assert.deepEqual(feedback.schaleIdleFeedbackImpacts(base, rewardImpactState), [
  { action: 'claim', label: '보상 수령', value: '+1개', tone: 'success' },
  { action: 'settle', label: '크레딧', value: '+180 Cr', tone: 'success' },
], '보상 수령은 청구 건수와 실제 크레딧을 함께 표시해야 합니다.');

assert.deepEqual(feedback.schaleIdleFeedbackImpacts(base, {
  ...base,
  log: ['녹슨 소총 제작 실패. 필요한 재료가 부족합니다.'],
}), [
  { action: 'craft', label: '제작 불가', value: '수치 변화 없음', tone: 'warning' },
], '실패 행동도 이전 변화량을 남기지 말고 수치 변화 없음으로 명시해야 합니다.');
assert.deepEqual(feedback.schaleIdleFeedbackImpacts(base, {
  ...dutyImpactState,
  runId: 'another-run',
}), [], '새 당직이나 불러오기는 이전 세이브와 변화량을 비교하면 안 됩니다.');

const impactSnapshot = feedback.createSchaleIdleFeedbackSnapshot(rewardImpactState);
assert.equal(impactSnapshot.claimCount, 1, '피드백 스냅샷은 수령한 보상 건수를 합산해야 합니다.');
assert.equal(impactSnapshot.equipmentCount, 0, '피드백 스냅샷은 보유 장비 수를 안정적으로 계산해야 합니다.');
...
assert.match(pageSource, /schaleIdleFeedbackImpacts\(previous, state\)/, '플레이 화면은 직전 상태와 현재 상태의 변화량을 계산해야 합니다.');
assert.match(pageSource, /SchaleIdleImpactStrip items=\{resultImpacts\}/, '최근 결과 아래에 변화량 아이콘 스트립을 표시해야 합니다.');
assert.match(pageSource, /feedbackRef\.current = restored;[\s\S]*setResultImpacts\(\[\]\)/, '불러오기 직후에는 이전 세이브 변화량을 초기화해야 합니다.');
...
assert.match(visualsSource, /export function SchaleIdleImpactStrip/, '샬레 전용 변화량 컴포넌트가 있어야 합니다.');
assert.match(visualsSource, /aria-label="최근 성장 변화량"/, '변화량 스트립은 보조 기술용 설명을 제공해야 합니다.');
assert.match(styleSource, /\.schale-idle-impact-strip\s*\{[\s\S]*grid-template-columns:/, '변화량 스트립은 안정적인 그리드 폭을 사용해야 합니다.');
assert.match(styleSource, /\.schale-idle-impact-chip\.is-danger/, '위험 결과 변화량은 별도 색상 톤을 사용해야 합니다.');
assert.match(styleSource, /@media \(max-width: 760px\)[\s\S]*\.schale-idle-impact-strip/, '모바일에서는 변화량 칩을 두 열로 재배치해야 합니다.');
const idleThemeSource = sfxSource.match(/idle:\s*\{([\s\S]*?)\n  \},\n  tactical:/)?.[1] || '';
['settle', 'dutyComplete', 'dutyPartial', 'dutyDefeat', 'towerClear', 'towerPartial', 'towerFail', 'craftComplete', 'enhanceSuccess', 'enhancePity', 'enhanceProtected', 'enhanceDowngrade', 'enhanceDestroyed', 'reward'].forEach((cue) => {
  assert.match(idleThemeSource, new RegExp(`${cue}:\\s*\\[`), `샬레 idle 테마에 ${cue} 전용 음색이 있어야 합니다.`);
});
assert.ok((idleThemeSource.match(/source: 'noise'/g) || []).length >= 12, '샬레 결과음은 타격과 반짝임을 위한 노이즈 레이어를 충분히 사용해야 합니다.');
assert.ok((idleThemeSource.match(/reverb:/g) || []).length >= 20, '샬레 결과음은 공간감을 위한 리버브 레이어를 충분히 사용해야 합니다.');
...
console.log(JSON.stringify({
  ...
  feedbackCues: 26,
  impactRows: dutyImpacts.length,
  idleThemeCues: (idleThemeSource.match(/^    [a-zA-Z]+:\s*\[/gm) || []).length,
