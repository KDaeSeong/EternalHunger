...
const campaignSource = await readFile(new URL('../src/app/games/ba-srpg/_components/BaSrpgCampaignExpansionTab.js', import.meta.url), 'utf8');
const visualsSource = await readFile(new URL('../src/app/games/ba-srpg/_components/BaSrpgVisuals.js', import.meta.url), 'utf8');
...
const objectivePresentation = baSrpgFeedbackPresentation({ ...base, latestLog: '[목표 확보: 정찰 표식] 시로코: 정찰 기록 확보' });
assert.equal(objectivePresentation.action, 'srpg-objective-command', '목표 확보는 전용 목표 아이콘을 사용해야 합니다.');
const objectivePendingPresentation = baSrpgFeedbackPresentation({ ...base, latestLog: '[목표 대기: 정찰 표식] 목표 타일 확보 필요' });
assert.equal(objectivePendingPresentation.action, 'srpg-objective-recon', '목표 대기는 미확보 목표 아이콘을 사용해야 합니다.');
assert.equal(objectivePendingPresentation.cue, 'objectivePending', '목표 대기는 일반 경고와 다른 전술 신호음을 사용해야 합니다.');
assert.equal(
  baSrpgFeedbackCue(base, { ...base, latestLog: '[목표 대기: 정찰 표식] 목표 타일 확보 필요' }),
  'objectivePending',
  '목표 대기 상태 전환은 전용 전술 신호음을 재생해야 합니다.',
);
assert.equal(
  baSrpgFeedbackCue(base, { ...base, objectiveCaptured: true, lastResult: '[목표 확보: 정찰 표식]' }),
  'objectiveCapture',
  '목표 확보는 전용 완료 효과음을 사용해야 합니다.',
);
assert.equal(
  baSrpgFeedbackCue(base, { ...base, missionEventKey: '2:sensor-smoke', missionEventCue: 'missionEventSmoke' }),
  'missionEventSmoke',
  '미션 사건은 사건 종류별 효과음을 사용해야 합니다.',
);
assert.equal(
  baSrpgFeedbackCue(base, { ...base, enemyPatternKey: '2:e_scout:targetLock', enemyPatternCue: 'enemyMark' }),
  'enemyMark',
  '적 전용 패턴은 패턴별 효과음을 사용해야 합니다.',
);
...
MISSIONS.forEach((mission) => {
  const objective = MISSION_OBJECTIVES[mission.id];
  assert.ok(objective, `${mission.name}에 목표 타일이 필요합니다.`);
  assert.ok(objective.x >= 0 && objective.x < GRID.width && objective.y >= 0 && objective.y < GRID.height, `${mission.name} 목표 타일은 전장 안에 있어야 합니다.`);
  const objectiveKey = `${objective.x},${objective.y}`;
  assert.equal(OBSTACLES.has(objectiveKey), false, `${mission.name} 목표 타일은 장애물과 겹치면 안 됩니다.`);
  assert.equal(COVER.has(objectiveKey), false, `${mission.name} 목표 타일은 엄폐물과 겹치면 안 됩니다.`);
  assert.equal(mission.enemies.some((enemy) => enemy.x === objective.x && enemy.y === objective.y), false, `${mission.name} 목표 타일은 초기 적 배치와 겹치면 안 됩니다.`);
  assert.ok((MISSION_EVENTS[mission.id] || []).length > 0, `${mission.name}에 미션 사건이 필요합니다.`);
  (MISSION_EVENTS[mission.id] || []).forEach((event) => {
    if (event.x === undefined || event.y === undefined) return;
    assert.ok(event.x >= 0 && event.x < GRID.width && event.y >= 0 && event.y < GRID.height, `${event.label} 좌표는 전장 안에 있어야 합니다.`);
  });

  const fresh = createNewState({ runId: `mission-data-${mission.id}` });
  const missionState = normalizeState({
    ...fresh,
    battle: {
      ...fresh.battle,
  ...
  assert.ok(missionState.battle.enemies.every((enemy) => ENEMY_PATTERNS[enemy.patternId]), `${mission.name}의 모든 적에 전용 패턴이 연결되어야 합니다.`);
  const objectiveCell = cellContent(missionState, objective.x, objective.y);
  assert.equal(objectiveCell.type, 'objective', `${mission.name} 목표 타일은 전투판 셀로 노출되어야 합니다.`);
  const overlay = getBattleMissionOverlay(missionState);
  assert.equal(overlay.objectiveStatus.id, objective.id, `${mission.name} 오버레이가 목표 상태를 제공해야 합니다.`);
  assert.equal(overlay.eventRows.length, MISSION_EVENTS[mission.id].length, `${mission.name} 오버레이가 사건 일정을 제공해야 합니다.`);
  assert.equal(overlay.enemyPatternRows.length, missionState.battle.enemies.length, `${mission.name} 오버레이가 생존 적 패턴을 제공해야 합니다.`);
});

const objectivePendingBase = tacticalState({
  runId: 'objective-clear-check',
  units: [combatActor('s_shiroko', 3, 0)],
  enemies: [],
  selectedUnitId: 's_shiroko',
});
const objectivePending = endTurnAction(objectivePendingBase);
assert.equal(objectivePending.battle.phase, 'player', '적을 전멸시켜도 목표 타일을 확보하기 전에는 클리어되면 안 됩니다.');
assert.match(objectivePending.battle.lastResult, /목표 대기/, '남은 목표를 전투 결과에 안내해야 합니다.');
const objectiveCaptured = moveSelectedAction(objectivePending, 1, 0);
assert.equal(objectiveCaptured.battle.objective.captured, true, '학생이 목표 타일에 들어가면 목표를 확보해야 합니다.');
assert.equal(objectiveCaptured.battle.phase, 'cleared', '적 전멸과 목표 확보를 모두 달성하면 임무가 완료되어야 합니다.');

const missionEventBase = tacticalState({
  runId: 'mission-event-check',
  turn: 2,
  units: [combatActor('s_hoshino', 0, 2, { hp: 80, maxHp: 80 })],
  enemies: [enemyActor('event-assault', 7, 5, { move: 0, patternId: 'assault' })],
  selectedUnitId: 's_hoshino',
});
const missionEventResolved = endTurnAction(missionEventBase);
assert.ok(missionEventResolved.battle.triggeredEventIds.includes('sensor-smoke'), '예정 턴의 미션 사건은 한 번 발생해야 합니다.');
assert.ok(missionEventResolved.battle.zones.some((zone) => zone.id === 'mission-event-sensor-smoke'), '센서 교란 사건은 실제 연막 지대를 만들어야 합니다.');
assert.ok(missionEventResolved.log.some((line) => line.includes('[미션 사건: 센서 교란]')), '미션 사건은 전투 로그에 남아야 합니다.');
const missionEventSecondTurn = endTurnAction(missionEventResolved);
assert.equal(missionEventSecondTurn.battle.triggeredEventIds.filter((id) => id === 'sensor-smoke').length, 1, '같은 미션 사건은 저장 상태에서 중복 발생하면 안 됩니다.');

const enemyPatternBase = tacticalState({
  runId: 'enemy-pattern-check',
  turn: 2,
  units: [combatActor('s_hoshino', 0, 2, { hp: 80, maxHp: 80 })],
  enemies: [enemyActor('pattern-drone', 3, 2, { move: 0, range: 3, patternId: 'targetLock' })],
  selectedUnitId: 's_hoshino',
  triggeredEventIds: ['sensor-smoke'],
});
const patternForecast = getBattleForecast(enemyPatternBase).enemyPlans[0];
assert.equal(patternForecast.patternId, 'targetLock', '적 턴 예측은 준비된 전용 패턴을 우선 표시해야 합니다.');
assert.equal(patternForecast.patternName, '표적 분석', '적 턴 예측에 패턴 이름이 필요합니다.');
const enemyPatternResolved = endTurnAction(enemyPatternBase);
assert.equal(enemyPatternResolved.battle.lastEnemyPattern?.patternId, 'targetLock', '예정 주기의 적 전용 패턴이 실제로 실행되어야 합니다.');
assert.equal(actorModifierValue(enemyPatternResolved.battle.units[0], 'Evasion'), -0.12, '표적 분석은 학생 회피를 12% 낮춰야 합니다.');
assert.ok(enemyPatternResolved.log.some((line) => line.includes('[적 스킬: 표적 분석]')), '적 전용 패턴은 전투 로그에 남아야 합니다.');
...
[
  'srpg-objective-recon',
  'srpg-objective-supply',
  'srpg-objective-route',
  'srpg-objective-breach',
  'srpg-objective-data',
  'srpg-objective-artillery',
  'srpg-objective-command',
  'srpg-event-smoke',
  'srpg-event-reinforcement',
  'srpg-event-supply',
  'srpg-event-command',
  'srpg-event-hazard',
  'srpg-enemy-mark',
  ...
  assert.ok(iconSource.includes(`'${action}':`), `${action} 목표, 사건, 패턴 아이콘이 필요합니다.`);
});
[
  'objectivePending',
  'objectiveCapture',
  ...
  assert.ok(sfxSource.includes(`${cue}: [`), `${cue} 미션 전용 효과음 프로필이 필요합니다.`);
});
assert.match(componentSource, /srpg-cell-objective-marker/, '목표 위에 유닛이 있어도 목표 아이콘을 표시해야 합니다.');
assert.match(componentSource, /battleMissionOverlay\.eventRows/, '미션 오버레이는 사건 일정을 표시해야 합니다.');
assert.match(componentSource, /battleMissionOverlay\.enemyPatternRows/, '미션 오버레이는 적 전용 패턴을 표시해야 합니다.');
assert.match(componentSource, /plan\.patternAction/, '적 턴 예상은 패턴별 아이콘을 표시해야 합니다.');
...
const visualSources = [componentSource, campaignSource, inventorySource, missionSource, townSource];
const semanticPanelTitles = visualSources.reduce((sum, source) => sum + [...source.matchAll(/<BaSrpgPanelTitle\b/g)].length, 0);
const semanticIconRows = visualSources.reduce((sum, source) => sum + [...source.matchAll(/<BaSrpgIconRow\b/g)].length, 0);
assert.equal(semanticPanelTitles, 32, 'BA SRPG의 32개 패널 제목에 전술 의미 아이콘이 있어야 합니다.');
assert.equal(semanticIconRows, 25, '전투, 캠페인, 장비 핵심 행 25곳에 의미 아이콘이 있어야 합니다.');
assert.ok(visualSources.every((source) => !source.includes('className="games-panel-title"')), '원시 패널 제목 마크업이 남아 있으면 안 됩니다.');
assert.ok(visualSources.every((source) => !source.includes('className="game-save-row"')), '핵심 목록이 의미 없는 원시 행으로 돌아가면 안 됩니다.');
assert.match(visualsSource, /export function BaSrpgPanelTitle/, 'SRPG 공용 패널 제목 컴포넌트가 필요합니다.');
...
console.log(JSON.stringify({
  ...
  missionEvents: Object.values(MISSION_EVENTS).flat().length,
  enemyPatterns: Object.keys(ENEMY_PATTERNS).length,
