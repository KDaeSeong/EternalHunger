...
const rendererSource = await readFile(
  path.join(clientRoot, 'scripts', 'render-si-coding-instrumental.mjs'),
  'utf8',
);
const manifest = JSON.parse(await readFile(
  path.join(publicRoot, 'audio', 'si-coding-sim', 'manifest.json'),
  'utf8',
));
...
assert.match(
  rendererSource,
  /theme: 'coding-success'[\s\S]*lead: 'trumpet'/,
  '그린 빌드에는 트럼펫 주선율이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'coding-delivery'[\s\S]*counter: 'bell'/,
  '릴리스 완료에는 완료 벨 편성이 필요합니다.',
);
