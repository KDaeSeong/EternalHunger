...
const operationsSource = await readFile(new URL('TonkatsuOperationsTab.js', componentUrl), 'utf8');
const productionSource = await readFile(new URL('TonkatsuProductionTab.js', componentUrl), 'utf8');
const tutorialSource = await readFile(new URL('TonkatsuTutorialTab.js', componentUrl), 'utf8');
const visualsSource = await readFile(new URL('TonkatsuVisuals.js', componentUrl), 'utf8');
...
expectFeedback(deliveryReady, delivery, { key: 'businessMode', cue: 'businessMode', action: 'tonkatsu-service', tone: 'highlight' });
...
const resultCues = [
  ...
  'trade', 'sales', 'businessMode', 'orderComplete', 'serve', 'upgrade', 'research', 'shop', 'equip',
...
for (const icon of [
  ...
  'combat', 'trophy', 'warning', 'new', 'reset', 'tournament', 'tonkatsu-service',
]) {
...
const semanticPanelTitles = (visualComponentsSource.match(/<TonkatsuPanelTitle\b/g) || []).length;
const semanticIconRows = (visualComponentsSource.match(/<TonkatsuIconRow\b/g) || []).length;
assert.equal(semanticPanelTitles, 28, '돈가스 선생의 28개 패널 제목에 의미 아이콘을 표시해야 합니다.');
assert.equal(semanticIconRows, 15, '돈가스 선생의 15개 반복 결과 행에 의미 아이콘을 표시해야 합니다.');
assert.doesNotMatch(visualComponentsSource, /className="games-panel-title"/, '기능 탭에 아이콘 없는 기존 패널 제목이 남으면 안 됩니다.');
assert.doesNotMatch(visualComponentsSource, /className="game-save-row"/, '기능 탭에 아이콘 없는 기존 결과 행이 남으면 안 됩니다.');
assert.match(visualsSource, /export function TonkatsuPanelTitle/, '돈가스 전용 패널 제목 컴포넌트가 있어야 합니다.');
assert.match(visualsSource, /export function TonkatsuIconRow/, '돈가스 전용 결과 행 컴포넌트가 있어야 합니다.');
assert.match(styleSource, /\.tonkatsu-panel-title h2/, '돈가스 패널 제목 아이콘 레이아웃이 있어야 합니다.');
assert.match(styleSource, /\.game-save-row\.tonkatsu-icon-row/, '돈가스 결과 행 아이콘 레이아웃이 있어야 합니다.');
