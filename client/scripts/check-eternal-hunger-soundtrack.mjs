...
assert.equal(soundtrack.ETERNAL_HUNGER_SOUNDTRACK.length, 9, '이터널 헝거 OST는 아홉 장면을 모두 가져야 합니다.');
assert.equal(
...
for (const track of soundtrack.ETERNAL_HUNGER_SOUNDTRACK) {
  ...
  assert.equal(profile.orchestration.leadStackGain > 0, true, `와이드 리드 스택이 누락됐습니다: ${track.theme}`);
  assert.equal(profile.orchestration.pluckGain > 0, true, `플럭 신스가 누락됐습니다: ${track.theme}`);
  assert.equal(profile.orchestration.timpaniGain > 0, true, `팀파니 레이어가 누락됐습니다: ${track.theme}`);
  assert.equal(profile.orchestration.hybridGain > 0, true, `하이브리드 임팩트가 누락됐습니다: ${track.theme}`);
  assert.equal(
...
assert.equal(resolve({ day: 2, phase: 'night', survivorCount: 1, isGameOver: true, hasWinner: true }), scenes.result);
assert.equal(resolve({ day: 2, phase: 'night', survivorCount: 0, isGameOver: true, hasWinner: false }), scenes.defeat);
assert.deepEqual(soundtrack.eternalHungerEventMusic({ action: 'rift-battle' }), { theme: scenes.rift, durationMs: 16_000 });
...
assert.match(providerSource, /scheduleSupersawLead/, '와이드 슈퍼소우 리드 스케줄러가 필요합니다.');
assert.match(providerSource, /scheduleSynthPluck/, '플럭 신스 스케줄러가 필요합니다.');
assert.match(providerSource, /scheduleGuitarStab/, '기타형 스탭 스케줄러가 필요합니다.');
assert.match(providerSource, /scheduleTimpani/, '팀파니 스케줄러가 필요합니다.');
assert.match(providerSource, /hybridGain/, '하이브리드 임팩트 레이어가 필요합니다.');
...
assert.match(viewSource, /eternalHungerEventMusic/, '특수 교전은 사건별 전용 트랙을 임시 재생해야 합니다.');
