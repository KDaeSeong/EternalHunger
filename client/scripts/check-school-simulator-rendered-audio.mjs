...
assert.match(
  rendererSource,
  /theme: 'school-morning'[\s\S]*lead: 'piano'[\s\S]*counter: 'bell'/,
  '아침 조회에는 피아노와 학교 종 편성이 필요합니다.',
);
assert.match(
  rendererSource,
...
assert.match(
  rendererSource,
  /theme: 'school-incident-alert'[\s\S]*lead: 'horn'[\s\S]*pad: 'choir'/,
  '학교 사건 장면에는 호른과 합창 편성이 필요합니다.',
);
assert.match(
  rendererSource,
  /theme: 'school-afterschool'[\s\S]*lead: 'celesta'[\s\S]*counter: 'guitar'/,
  '방과 후 장면에는 첼레스타와 기타 편성이 필요합니다.',
);
