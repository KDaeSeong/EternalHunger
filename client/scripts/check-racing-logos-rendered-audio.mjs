...
const rendererSource = await readFile(
  path.join(clientRoot, 'scripts', 'render-racing-logos-instrumental.mjs'),
  'utf8',
);
...
const manifest = JSON.parse(await readFile(
  path.join(publicRoot, 'audio', 'racing-logos-demo', 'manifest.json'),
  'utf8',
));
...
assert.equal(
  ...
  7,
  'Racing Logos 렌더 트랙 매핑은 7곡이어야 합니다.',
);
...
for (const soundtrackRow of RACING_LOGOS_SOUNDTRACK) {
  ...
  assert.equal(
    durationSeconds >= 65 && durationSeconds <= 100,
    true,
