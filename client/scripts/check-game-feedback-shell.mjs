...
assert.match(soundControlSource, /games-audio-menu__now-playing/, '공용 오디오 메뉴는 현재 곡을 표시해야 합니다.');
assert.match(soundControlSource, /musicIcon/, '공용 오디오 메뉴는 현재 곡의 장면 아이콘을 표시해야 합니다.');
...
for (const theme of expectedBgmThemes) {
  ...
  assert.equal(profile.icon.length > 0, true, `BGM profile scene icon is required: ${theme}`);
