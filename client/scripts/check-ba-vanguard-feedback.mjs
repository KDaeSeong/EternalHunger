...
function expectResult(previous, current, expected) {
  ...
  assert.ok(Array.isArray(presentation.impacts), `${expected.key} 결과에 수치 변화 배열이 있어야 합니다.`);
  return presentation;
}
...
const drawPresentation = expectResult(base, drawState, { key: 'draw', action: 'vanguard-draw', cue: 'vanguardDraw', tone: 'success' });
assert.ok(drawPresentation.impacts.some((item) => item.label === '내 패' && item.value === '+1'), '드로우 결과는 내 패 증가를 표시해야 합니다.');
assert.ok(drawPresentation.impacts.some((item) => item.label === '내 덱' && item.value === '-1'), '드로우 결과는 내 덱 감소를 표시해야 합니다.');
...
assert.ok(assistedRidePresentation.impacts.some((item) => item.label === '내 소울' && item.value === '+1'), '라이드 결과는 소울 증가를 표시해야 합니다.');
assert.ok(assistedRidePresentation.impacts.some((item) => item.label === '내 패' && item.value === '-1'), '라이드 결과는 패 감소를 표시해야 합니다.');
...
expectResult(mainState, invalidGradeRide, { key: 'rideBlocked', action: 'vanguard-ride', cue: 'vanguardRideBlocked', tone: 'warning' });
...
const guardedPresentation = expectResult(guardWindow, guarded, { key: 'guardAdded', action: 'vanguard-guard', cue: 'vanguardGuard', tone: 'success' });
assert.ok(guardedPresentation.impacts.some((item) => item.label === '가드 실드' && /^\+/.test(item.value)), '가드 결과는 추가된 실드를 표시해야 합니다.');
...
const skillBlocked = clone(mainState);
skillBlocked.log = ['[나] VC 스킬 비용으로 사용할 패 또는 소울이 없습니다.', ...skillBlocked.log];
expectResult(mainState, skillBlocked, { key: 'skillBlocked', action: 'vanguard-skill', cue: 'vanguardSkillBlocked', tone: 'warning' });
const guardBlocked = clone(mainState);
guardBlocked.log = ['[나] 현재 사용할 수 있는 G 가디언 또는 비용 카드가 없습니다.', ...guardBlocked.log];
expectResult(mainState, guardBlocked, { key: 'guardBlocked', action: 'vanguard-guard', cue: 'vanguardGuardBlocked', tone: 'warning' });
const attackDenied = clone(mainState);
attackDenied.log = ['[나] 선공 첫 턴에는 공격할 수 없습니다.', ...attackDenied.log];
expectResult(mainState, attackDenied, { key: 'attackDenied', action: 'vanguard-attack', cue: 'vanguardAttackDenied', tone: 'warning' });
...
assert.match(pageSource, /<BaVanguardImpactStrip items=\{resultPresentation\.impacts\}/, '상단 결과 아래에 실제 수치 변화 띠를 표시해야 합니다.');
assert.doesNotMatch(pageSource, /myanime 원본|P-G 플레이테스트/, '사용자 화면에 내부 이식 단계 문구가 노출되면 안 됩니다.');
...
assert.match(duelSource, /guardAudit=\{replayReport\.guardAudit\}/, '듀얼 탭은 실시간 가드 계산을 배틀 패널에 전달해야 합니다.');
assert.match(boardSource, /<BaVanguardBattleForecast audit=\{guardAudit\}/, '배틀 패널은 공격·방어·가용 실드 예측을 표시해야 합니다.');
...
assert.match(visualSource, /export function BaVanguardImpactStrip/, '결과 수치 변화 띠 컴포넌트가 있어야 합니다.');
assert.match(visualSource, /export function BaVanguardBattleForecast/, '전투 예측 컴포넌트가 있어야 합니다.');
...
assert.match(styleSource, /\.ba-vanguard-impact-strip__item/, '수치 변화 띠 스타일이 있어야 합니다.');
