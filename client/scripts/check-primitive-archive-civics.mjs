...
const expansionPassives = [...classicalTechnologies, ...classicalCivics, ...earlyModernTechnologies, ...earlyModernCivics]
...
assert.equal(technologyMap.maxTier, 22, '기술 트리는 T22에서 끝나야 합니다.');
assert.equal(technologyMap.rangeLabel, 'T1-T22', '기술 트리는 T 접두사를 사용해야 합니다.');
assert.equal(technologyMap.tierCount, 22, '기술 트리는 스물두 단계가 모두 표시되어야 합니다.');
assert.deepEqual(
...
assert.equal(civicMap.maxTier, 20, '사회 제도 트리는 C20에서 끝나야 합니다.');
assert.equal(civicMap.rangeLabel, 'C1-C20', '사회 제도 트리는 C 접두사를 사용해야 합니다.');
assert.equal(civicMap.tierCount, 20, '사회 제도 트리는 스무 단계가 모두 표시되어야 합니다.');
