      ...
      <section className="games-panel games-action-dock primitive-action-workspace">
        ...
        <div className="primitive-specialized-actions primitive-utility-actions">
          <div className="primitive-specialized-actions__header">
            ...
            <span>{utilityRows.filter((row) => row.unlocked).length}/{utilityRows.length} 해금 · 순찰 {Number(state.exploration?.patrolCharges || 0)}/2 · 관개 {Number(state.exploration?.irrigationCharges || 0)}/3 · 도로 {Number(state.exploration?.roadCharges || 0)}/4</span>
          </div>
          <div className="primitive-specialized-actions__buttons">
            {utilityRows.map((row) => (
              <ActionButton
