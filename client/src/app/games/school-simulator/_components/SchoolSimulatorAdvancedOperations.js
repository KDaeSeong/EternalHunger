  ...
  return (
    ...
    <section className="games-detail-grid">
                <section className="games-panel">
                  ...
                  <ActionButton cue="off" onClick={applySelectedAction} disabled={state.player.weeklyActionPoint < selectedAction.apCost}>
                    {selectedAction.label} 실행
                  </ActionButton>
                  <SchoolSimulatorActionResult fallbackLabel="최근 주간 운영 결과" resultPresentation={resultPresentation} text={recentActionText} />
                </section>
                ...
                <section className="games-panel">
                  ...
                  <ActionButton action="policy" cue="off" onClick={applySelectedPolicy} disabled={state.player.weeklyActionPoint < 1 || state.school.policyPreset === policyId}>
                    정책 적용
                  </ActionButton>
                  <SchoolSimulatorActionResult fallbackLabel="최근 정책 결과" resultPresentation={resultPresentation} text={recentActionText} />
                </section>
                ...
                <section className="games-panel">
                  ...
                  <div style={{ display: 'grid', gap: 8 }}>
                    <ActionButton action="advance" cue="off" onClick={() => applySchoolAction('다음 주 진행', (current) => endWeekAction(current))}>다음 주로 진행</ActionButton>
                    <ActionButton action="rest" cue="off" onClick={() => applySchoolAction('운영진 휴식', (current) => restAction(current))} disabled={state.player.weeklyActionPoint < 2}>지친 운영진 휴식</ActionButton>
                  </div>
                  <SchoolSimulatorActionResult fallbackLabel="최근 정산 결과" resultPresentation={resultPresentation} text={recentActionText} />
                </section>
              </section>
    ...
    <section className="games-detail-grid">
                <section className="games-panel">
                  ...
                  <div style={{ display: 'grid', gap: 8 }}>
                    <ActionButton action="lesson" cue="off" onClick={() => applySchoolAction('수업 방식 변경', (current) => applySubjectPolicyAction(current, subjectId, subjectModeId))} disabled={state.player.weeklyActionPoint < 1}>수업 방식 변경</ActionButton>
                    <ActionButton
                      action="festival"
                      cue="off"
                  ...
                  <SchoolSimulatorActionResult fallbackLabel="최근 과목 운영 결과" resultPresentation={resultPresentation} text={recentActionText} />
                ...
                <section className="games-panel">
                  ...
                  <ActionButton action="recruit" cue="off" onClick={() => applySchoolAction('모집 캠페인', (current) => runAdmissionCampaignAction(current, recruitmentStrategyId))} disabled={state.player.weeklyActionPoint < 2}>
                    모집 캠페인
                  </ActionButton>
                  <SchoolSimulatorActionResult fallbackLabel="최근 모집 결과" resultPresentation={resultPresentation} text={recentActionText} />
                </section>
                ...
                <section className="games-panel">
                  ...
                  <ActionButton action="counsel" cue="off" onClick={() => applySchoolAction('진로 상담', (current) => runCareerCounselingAction(current, careerTrackId))} disabled={state.player.weeklyActionPoint < 2}>
                    진로 상담 실행
                  </ActionButton>
                  <SchoolSimulatorActionResult fallbackLabel="최근 진로 지도 결과" resultPresentation={resultPresentation} text={recentActionText} />
                </section>
              </section>
    </>
  );
