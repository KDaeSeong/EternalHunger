...
function matchDrama(result, sets) {
  const winnerTeamId = String(result?.winnerTeamId || '');
  const scoreHome = Math.max(0, Number(result?.scoreHome || 0));
  const scoreAway = Math.max(0, Number(result?.scoreAway || 0));
  const winnerScore = winnerTeamId === String(result?.homeTeamId || '') ? scoreHome : scoreAway;
  const loserScore = winnerTeamId === String(result?.homeTeamId || '') ? scoreAway : scoreHome;
  const expectedSetCount = scoreHome + scoreAway;
  const completeSetLog = expectedSetCount > 0 && sets.length >= expectedSetCount;
  const leadingLosses = sets.findIndex((setResult) => String(setResult?.winnerTeamId || '') === winnerTeamId);
  const reverseSweep = completeSetLog
    && winnerScore >= 2
    && loserScore === winnerScore - 1
    && leadingLosses === loserScore
    && sets.slice(leadingLosses).every((setResult) => String(setResult?.winnerTeamId || '') === winnerTeamId);

  return {
    sweep: completeSetLog && winnerScore >= 2 && loserScore === 0,
    reverseSweep,
    decider: completeSetLog && expectedSetCount >= 3 && Math.abs(scoreHome - scoreAway) === 1,
  };
}
...
function latestPlayedMatch(state) {
  ...
  const drama = matchDrama(result, sets);
  return {
    ...
    sweep: drama.sweep,
    reverseSweep: drama.reverseSweep,
    decider: drama.decider,
...
export function starleagueFeedbackSnapshot(state) {
  ...
  return {
    ...
    latestSweep: Boolean(latestMatch?.sweep),
    latestReverseSweep: Boolean(latestMatch?.reverseSweep),
    latestDecider: Boolean(latestMatch?.decider),
...
export function starleagueFeedbackPresentation(snapshot, selectedTeamId = '') {
  ...
  if (involved) {
    ...
    if (snapshot.latestReverseSweep) {
      return {
        action: 'starleague-reverse-sweep',
        cue: won ? 'starleagueReverseSweep' : 'defeat',
        label: won ? '응원팀 역스윕' : '응원팀 역스윕 허용',
        tone: won ? 'success' : 'danger',
        detail: matchDetail(snapshot),
      };
    }
    if (snapshot.latestSweep) {
      return {
        action: 'starleague-sweep',
        cue: won ? 'starleagueSweep' : 'defeat',
        label: won ? '응원팀 완봉승' : '응원팀 완봉패',
        tone: won ? 'success' : 'danger',
        detail: matchDetail(snapshot),
      };
    }
    if (snapshot.latestDecider) {
      return {
        action: 'starleague-clutch',
        cue: won ? 'starleagueClutch' : 'defeat',
        label: won ? '응원팀 최종 세트 승리' : '응원팀 최종 세트 패배',
        tone: won ? 'success' : 'danger',
        detail: matchDetail(snapshot),
      };
    }
  ...
  if (snapshot.latestReverseSweep) {
    return { action: 'starleague-reverse-sweep', cue: 'starleagueReverseSweep', label: '역스윕', tone: 'success', detail: matchDetail(snapshot) };
  }
  if (snapshot.latestHasAceSet) {
    return { action: 'verdict', cue: 'verdict', label: '에이스 결정전', tone: 'highlight', detail: matchDetail(snapshot) };
  }
  if (snapshot.latestSweep) {
    return { action: 'starleague-sweep', cue: 'starleagueSweep', label: '완봉승', tone: 'highlight', detail: matchDetail(snapshot) };
  }
  ...
  if (snapshot.latestDecider) {
    return { action: 'starleague-clutch', cue: 'starleagueClutch', label: '최종 세트 승부', tone: 'highlight', detail: matchDetail(snapshot) };
  }
