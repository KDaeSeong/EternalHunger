...
export function pickKioskExchangeAction({
  ...
  if (canExchangeForceCore && simulationRandom() < 0.42) {
  ...
  if (canExchangeMithril && simulationRandom() < 0.38) {
  ...
  if (tacUpgradeMode !== 'level') {
    if (tacModuleItem && tacModuleHave >= 2 && simulationRandom() < 0.55) {
  ...
  } else if (tacModuleItem && tacSkillLv >= tacMaxLevel && tacModuleHave >= 1 && simulationRandom() < 0.25) {
    const gain = itemCreditPrice(tacModuleItem, 100);
  ...
  if (meteorItem && lifeTreeItem) {
    if (needTree && has(meteorItem, 1) && simulationRandom() < 0.75) {
    ...
    if (needMeteor && has(lifeTreeItem, 1) && simulationRandom() < 0.75) {
